...
`requirements-dev.lock` is generated, not hand-edited: `requirements-dev.in` is its committed source, and `scripts/relock-dev.py` compiles the one from the other against the versions the released images actually ship. That indirection is the point. The test environment exists to import the code that runs inside the agent and mcp images, so resolving it freely against PyPI is how it silently ends up two `langchain-core` minors ahead of the container — while `concierge/` imports that package's message and chat-model classes directly, and a green suite stops being evidence about the thing that ships. The script layers the three images' own locks, `agent` over `mcp` over `chatbot`, because they disagree with each other and one virtualenv cannot mirror all three; it prints the packages where that choice loses, and it refuses if the lock it just wrote does not match the pins it just read. Only `pytest`, `pytest-asyncio` and `ruff` float, since no image pins them. Re-run it after a `COMMIT` bump in `launcher/core.py`, which is what moves the pins underneath it.
...
- Upstream: [3.0.0](https://github.com/open-telemetry/opentelemetry-demo/releases/tag/3.0.0), commit `1755859a9de82c2e5e225be68abc401a5ebf2b4f`, for both targets. The demo Helm chart is `open-telemetry/opentelemetry-demo` 0.41.2, whose `appVersion` is the same 3.0.0.
