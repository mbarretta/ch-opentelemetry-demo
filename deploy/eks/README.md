...
| **Langfuse stays external** | Langfuse Cloud serves both targets, so nothing in the cluster stores assistant traces and the EKS and laptop runs land in the same project. Langfuse is required here, the same as ClickStack: `eks deploy` refuses, naming whichever key is blank, before creating the `langfuse-credentials` Secret or any other Secret. |
